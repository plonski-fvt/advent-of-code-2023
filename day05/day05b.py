...
seeds = [Seed(int(x[0]), int(x[1])) for x in seeds_as_strs]
...
closest = 99999999999
for seed in seeds:
    for id in range(seed.start_id, seed.start_id + seed.range):
        location = map_seed_to_location(id, sequential_maps)
        if location < closest:
            closest = location
    print(closest)
